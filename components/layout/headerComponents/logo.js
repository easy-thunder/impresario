...
export default function Logo({setModal, modal}){
    ...
    async function chatHelper (){
        setChat(()=>true)
        const action = "CHAT"
        await fetch(`/api/auth/UserIndividualActions/${encodeURIComponent(session.user.email)}`,
        {method: "PATCH",
    ...
    return <Fragment>
        <svg onClick={chatHelper} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="chatIcon">
